...
public class BanzaiApplication implements Application {
    ...
    public void send42(Order order) {
        ...
        newOrderSingle.set(new OrderQty(order.getQuantity()));
        newOrderSingle.set(new SecurityExchange(order.getExchId().toString()));//207
        send(populateOrder(order, newOrderSingle), order.getSessionID());
    }
...
//
////        newOrderSingle.set(new OrderRestrictions("2")); //529
//        newOrderSingle.setChar(LotType.FIELD,'1');// special/odd lot test
        send(populateOrder(order, newOrderSingle), order.getSessionID());
    ...
    public quickfix.Message populateOrder(Order order, quickfix.Message newOrderSingle) {
        ...
        ExchIdType exchIdType = order.getExchId();
    ...
    public void cancel40(Order order) {
        ...
        quickfix.fix40.OrderCancelRequest message = new quickfix.fix40.OrderCancelRequest(
                new OrigClOrdID(order.getID()), new ClOrdID(id), new CxlType(CxlType.FULL_REMAINING_QUANTITY),
                new Symbol(order
    ...
    public void cancel50(Order order) {
        String id = order.generateID();
        quickfix.fix50sp2.OrderCancelRequest message = new quickfix.fix50sp2.OrderCancelRequest(
                new ClOrdID(id), sideToFIXSide(order.getSide()), new TransactTime());
        message.set(new Symbol(order.getSymbol()));
        message.setField((new OrigClOrdID(order.getID())));
        message.setField(new OrderQty(order.getQuantity()));
        message.setField(new SecurityID("8"));
        orderTableModel.addID(order, id);
        send(message, order.getSessionID());
    }
    ...
    public void replace50(Order order, Order newOrder) {
        ...
        quickfix.fix50sp2.OrderCancelReplaceRequest message =
                new quickfix.fix50sp2.OrderCancelReplaceRequest(new ClOrdID(order.getID()),
                        sideToFIXSide(order.getSide()),
                        new TransactTime(), typeToFIXType(order.getType()));
